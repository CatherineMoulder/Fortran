...
! Opening files for read/write	
  ...
  open(unit=12, file='PolarizationSnapshot.out', status='new') !opens an output file to write to
...
!     & molnum, x(i), y(i), z(i)   
	 if (natom == 'CONECT') then ! Trimming the input read so that it is only positions.
	 exit
	 end if 
...
! Setting the itterators  
  N = i - 1 ! i is itterating over atoms
  write(*,*) 'N = ', N 
  j = 1 ! j itterates over molecules, 3 atoms (i) per H-O-H molecule
...
!	This gives us the running sum of the molecular polarizabilities	Pi(M)
	...
	j = j + 1	! itterating over the number of molecules, not atoms
	...
	do k = 1, j-1 ! start at one and go through each molecule, i in the lit.
		...
		call PBCcheck1(prox, HoldMassX, HoldMassY, HoldMassZ, Move)
			...
			do l = k+1, j ! all post-k molecules, l is j in the lit for our Trij tensor
				...
				if (Move .eqv. .true.) then ! Checking for potential interactions across the pbc
					call PBCcheck2 (prox,pbcx, pbcy, pbcz, HoldMassX, HoldMassY, HoldMassZ, &
								VaryMassX, VaryMassy, VaryMassZ, NVarx, NVary, NVarz)
						VaryMassX= NVarx
						VaryMassy= NVary
						VaryMassZ= NVarz
				endif	! Only translate the molecules we must.		
...
! 16000					format(A17,1X, I4,4X,I4,4X, 3(F15.3),/, 34x,3(F15.3),/, 34x,3(F15.3),/) 
		...
		end do ! finishing inner loop over k
...
end program PolarizationSnapshot
